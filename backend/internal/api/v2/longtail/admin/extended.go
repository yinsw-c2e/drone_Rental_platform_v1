...
func (h *Handler) RegisterExtendedRoutes(group *gin.RouterGroup) {
	...
	group.GET("/service-classes", h.ServiceClassList)
	group.POST("/service-classes", h.CreateServiceClass)
	group.PUT("/service-classes/:id", h.UpdateServiceClass)
	group.DELETE("/service-classes/:id", h.ArchiveServiceClass)
	group.GET("/system-configs", h.SystemConfigList)
	group.GET("/system-configs/:key", h.GetSystemConfig)
	group.PUT("/system-configs/:key", h.UpdateSystemConfig)
	group.GET("/broadcasts/stats", h.BroadcastStats)
	group.GET("/broadcasts/recent", h.BroadcastRecent)
