...
## 更新摘要
**变更内容**
- MapPickerScreen 新增自定义头部返回按钮系统（包含箭头图标和'返回'文本标签）
- 新增安全退出逻辑（beginSafeExit机制），防止原生组件崩溃和内存泄漏
- 状态管理优化（isClosing标志防止多重导航尝试）
- Android Fragment工厂集成优化屏幕过渡和内存管理
- 更新Android退出延迟常量从220ms调整为400ms
...
- 地图选点（MapPickerScreen）：集成高德地图SDK，支持拖拽定位、周边POI展示、回到当前位置、坐标拾取，具备安全退出机制防止原生组件崩溃。
...
## 架构总览
移动端通过locationService调用后端位置接口，后端通过amap服务封装调用高德地图API；定位能力由LocationService统一处理，坐标转换遵循中国地图标准（GCJ-02）。地图选点界面集成react-native-amap3d，支持拖拽、周边POI查询与地址反编译，具备完善的安全退出机制。
...
CheckAllow --> |否| CheckClosing{"isClosing中?"}
CheckClosing --> |是| Return["直接返回"]
CheckClosing --> |否| SetClosing["设置isClosing=true"]
SetClosing --> HideMap["setMapVisible=false"]
HideMap --> ClearTimers["清理fetchTimer和exitTimer"]
ClearTimers --> StartDelay["启动延迟定时器<br/>MAP_EXIT_DELAY_MS"]
StartDelay --> DelayEnd["延迟结束"]
DelayEnd --> AllowRemove["allowRemoveRef=true"]
...
**图表来源**
- [mobile/src/screens/location/MapPickerScreen.tsx:61-89](file://mobile/src/screens/location/MapPickerScreen.tsx#L61-L89)
- [mobile/src/screens/location/MapPickerScreen.tsx:92-106](file://mobile/src/screens/location/MapPickerScreen.tsx#L92-L106)
...
- 前端依赖
  ...
  - MapPickerScreen依赖react-native-amap3d与LocationService，具备安全退出机制
...
- **安全退出问题**：
  ...
  - 确认 `MAP_EXIT_DELAY_MS` 常量在不同平台的配置正确
  - 检查 `allowRemoveRef` 和 `isClosing` 状态的同步
- **自定义头部问题**：
  - 检查返回按钮的样式和交互是否正常
  - 确认在关闭过程中按钮被正确禁用
- **导航配置问题**：
...
- MapPickerScreen 新增安全退出机制，有效防止原生组件崩溃和内存泄漏
- Android平台的退出延迟从220ms调整为400ms，提升稳定性
