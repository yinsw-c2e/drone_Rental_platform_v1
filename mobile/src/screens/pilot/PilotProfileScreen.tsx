...
const buildDraftFromProfile = (profile: any) => ({
  ...
  service_radius: String(
    profile.service_radius_km || Math.round(profile.service_radius || 50) || 50,
  ),
...
export default function PilotProfileScreen({ navigation }: any) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setPilot(profile);
      if (profile && !draftDirtyRef.current) {
  ...
  const toggleAvailability = useCallback(
    async (enabled: boolean) => {
      ...
      try {
        const res = await pilotV2Service.updateAvailability(
    ...
    [draft.service_base_latitude, draft.service_base_longitude, pilot],
  );

  const chooseServiceBase = useCallback(() => {
    skipNextFocusReloadRef.current = true;
    navigation.navigate('MapPicker', {
      latitude: draft.service_base_latitude || undefined,
      longitude: draft.service_base_longitude || undefined,
      onSelect: (addr: any) => {
        draftDirtyRef.current = true;
        const city = pickLocationCity(addr.city, addr.district, addr.province);
        setDraft(prev => ({
          ...prev,
          current_city: city,
          service_base_address: addr.address || addr.name || '',
          service_base_latitude: Number(addr.latitude || 0),
          service_base_longitude: Number(addr.longitude || 0),
        }));
      },
    });
  }, [draft.service_base_latitude, draft.service_base_longitude, navigation]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const res = await pilotV2Service.upsertProfile(payload);
      const savedProfile = {
        ...res.data,
  ...
  const handleEnterFlightMonitoring = useCallback(async () => {
    try {
      ...
      const activeTask = (res.data?.items || []).find(
        (item: any) =>
          item.order?.id &&
          ...
          <View style={styles.readinessCard}>
            <View style={styles.readinessHeader}>
              <View style={styles.readinessInfo}>
                <Text style={styles.readinessTier}>
        ...
        <ObjectCard style={styles.settingsCard}>
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput
              ...
              onChangeText={text => {
                draftDirtyRef.current = true;
                setDraft(prev => ({ ...prev, service_radius: text }));
              }}
            />
          </View>
                ...
                return (
                  ...
                  >
                    <Text
                      style={[
                        styles.skillBtnText,
                        active && styles.skillBtnTextActive,
                      ]}
                    >
                      {skill}
                    </Text>
