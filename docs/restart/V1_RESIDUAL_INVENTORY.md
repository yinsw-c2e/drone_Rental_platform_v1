...
- 本轮已经额外收口了两类高风险残留：
  - 移动端默认 API 地址不再硬编码到 `cpolar + /api/v2`，而是先落到 `/api` 根路径，再按客户端切到 `v1 / v2`
...
| 信用 / 保险 / 分析后台 | `backend/internal/api/v2/credit|insurance|analytics` | 暂无对应 v2 域 | 保留 |
...
- `cpolar/api/v2` 默认地址已收口到 `/api` 根路径，避免新客户端默认打回 `v1`
